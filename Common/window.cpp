...
void Window::SetWindowCallbacks()
{
    ...
    glfwSetKeyCallback(window,
        [](GLFWwindow* win, int key, int scancode, int action, int mods) {
            ...
            for (const auto& handler : window->keyboardKeyHandlers) {
                handler(key, action);
            }
        }
    );
...
void Window::AddKeyboardKeyHandler(const KeyboardKeyHandler& handler)
{
    keyboardKeyHandlers.push_back(handler);
}
